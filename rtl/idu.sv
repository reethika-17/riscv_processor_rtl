...
`define SIGN_EXTEND(SIG, WIDTH) {{ (32 - WIDTH) {SIG[WIDTH-1]} }, SIG}
...
always @ (*) begin
    ...
    case (instruction_code[6:0])
        ...
        `OP_IMM: begin
            rs1 = imm_inst_rs1;
            rd = imm_inst_rd;
            imm = `SIGN_EXTEND(imm_inst_imm, 12);
            alu_control = imm_inst_alu_control;
        end
        `OP_LOAD: begin
            rs1 = load_inst_rs1;
            rd = load_inst_rd;
            imm = `SIGN_EXTEND(load_inst_imm, 12);
            load_control = w_load_control;
        end
        `OP_STORE: begin
            rs1 = store_inst_rs1;
            rs2 = store_inst_rs2;
            imm = `SIGN_EXTEND(store_inst_imm, 12);
            store_control = w_store_control;
        end
        `OP_BRANCH: begin
            rs1 = branch_inst_rs1;
            rs2 = branch_inst_rs2;
            imm = `SIGN_EXTEND(branch_inst_imm, 13);
            branch_control = w_branch_control;
        end
        `OP_JAL: begin
            rd = jump_inst_rd;
